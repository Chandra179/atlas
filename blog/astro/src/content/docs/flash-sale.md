...
* **Cache Layer:** 3-node Redis cluster (1 master, 2 replicas), master CPU ≤80% — single-threaded, so this is a hard wall.
...
3. **Cross-bucket dedupe:** because the same `user_id + product_id` dedupe key is checked before every bucket attempt, a retry that falls through to a second bucket after the first already succeeded is rejected — no double-decrement across buckets.
