...
*  **Database Sharding:** Shard Post DB by `post_id` (not `user_id`). The dominant access pattern is point lookups by `post_id` during feed hydration; sharding by `user_id` would create hot shards for celebrity accounts.
