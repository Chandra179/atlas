...
- `scripts/sync-content.mjs` runs before every `dev`/`build`. It copies a fixed allow-list of `*.md` files from the repo (e.g. `about/introduction.md`, `system-design/cache.md`, `swe/swe-journey.md`), flattening subdirectory files to their basename, into `src/content/docs/`, preserving/merging frontmatter and stamping a `modified` date. It also deletes anything in `src/content/docs/` not on the allow-list (including subdirectories) — so **content is authored in the topic directories at the repo root**, not directly under `src/content/docs/`, and edits to `src/content/docs/*.md` will be overwritten/removed on the next sync unless the file is added to `ALLOWED_FILES` in that script.
...
- remark: math → GitBook embeds → link rewriting → directives → callouts
...
- Results are cached in the `PDF_CACHE` KV namespace for 24h (`CACHE_KEY_PREFIX = 'pdf:v12:'` — bump this prefix if the cleaning/rendering logic changes incompatibly, to avoid serving stale cached PDFs).
